...
def streaming_xtts(default_config: ConfigLoader, monkeypatch) -> XTTS:
    monkeypatch.setattr(requests.Session, 'get', lambda self, *args, **kwargs: FakeXTTSServerResponse(['maleeventoned']))
    monkeypatch.setattr(requests.Session, 'post', lambda self, *args, **kwargs: FakeResponse())
