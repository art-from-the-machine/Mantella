...
class ChatManager:
    def __init__(self, game_state_manager, config, encoding, tts: Synthesizer, client: openai_client):
    ...
    async def get_response(self, messages: message_thread, characters: set[Character], radiant_dialogue: bool) -> message_thread:
        sentence_queue: asyncio.Queue[tuple[str,str]] = asyncio.Queue()
    ...
    async def send_response(self, sentence_queue: asyncio.Queue[tuple[str,str]], event: asyncio.Event):
    ...
    async def process_response(self, sentence_queue: asyncio.Queue[tuple[str,str]], messages : message_thread, characters: set[Character], radiant_dialogue: bool, event:asyncio.Event) -> message_thread:
                        ...
                        if (any(char in content_edit for char in self.end_of_sentence_chars)):
                            ...
                            if action_taken == False:
                                ...
                                try:
                                    ...
                                    audio_file = self.__tts.synthesize(self.active_character.voice_model, None, ' ' + sentence + ' ', self.active_character.is_in_combat)
