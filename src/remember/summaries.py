...
class summaries(remembering):
    ...
    """
    def __init__(self, client: openai_client, language_name: str, summary_limit_pct: float = 0.45) -> None:
        super().__init__()
    ...
    def __create_new_conversation_summary(self, messages: message_thread, npc_name: str) -> str:
        prompt = f"You are tasked with summarizing the conversation between {npc_name} (the assistant) and the player (the user) / other characters. These conversations take place in Skyrim. It is not necessary to comment on any mixups in communication such as mishearings. Text contained within asterisks state in-game events. Please summarize the conversation into a single paragraph in {self.__language_name}."
        while True:
            try:
                if len(messages) > 5:
    ...
    def __append_new_conversation_summary(self, new_summary: str, character_to_save_for: Character):
        # if this is not the first conversation
        if os.path.exists(character_to_save_for.conversation_summary_file):
            with open(character_to_save_for.conversation_summary_file, 'r', encoding='utf-8') as f:
                previous_conversation_summaries = f.read()
        # if this is the first conversation
        else:
            directory = os.path.dirname(character_to_save_for.conversation_summary_file)
            os.makedirs(directory, exist_ok=True)
            previous_conversation_summaries = ''
       
        conversation_summaries = previous_conversation_summaries + new_summary
        with open(character_to_save_for.conversation_summary_file, 'w', encoding='utf-8') as f:
        ...
        if count_tokens_summaries > summary_limit:
            ...
            while True:
                try:
                    prompt = f"You are tasked with summarizing the conversation history between {character_to_save_for.name} (the assistant) and the player (the user) / other characters. These conversations take place in Skyrim. "\
                        f"Each paragraph represents a conversation at a new point in time. Please summarize these conversations into a single paragraph in {self.__language_name}."
                    long_conversation_summary = self.summarize_conversation(conversation_summaries, prompt, character_to_save_for.name)
                    break
                except:
                    logging.error('Failed to summarize conversation. Retrying...')
                    time.sleep(5)
            ...
            base_directory, filename = os.path.split(character_to_save_for.conversation_summary_file)
            ...
            character_to_save_for.conversation_summary_file = character_to_save_for.get_latest_conversation_summary_file_path()
