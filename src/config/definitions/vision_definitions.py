...
class VisionDefinitions:
    ...
    def get_vision_llm_api_config_value() -> ConfigValue:
        ...
        # --- PLAYER2 START ---
        # "Player2" added as a supported vision model service.
        # Player2 accepts image_url content blocks in the same format as OpenAI,
        # so no message format changes are needed — only the 'detail' field must
        # be omitted as Player2 does not support it (handled in image_client.py).
        options = ["OpenRouter", "OpenAI", "NanoGPT", "KoboldCpp", "textgenwebui", "Player2"]
        # --- PLAYER2 END ---
