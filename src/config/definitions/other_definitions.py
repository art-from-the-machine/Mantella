...
class OtherDefinitions:
    ...
    def get_remove_mei_folders_config_value() -> ConfigValue:
        ...
        return ConfigValueBool("remove_mei_folders","Remove mei folders",remove_mei_folders_description,False)
    
    #HTTP
    @staticmethod
    def get_port_config_value() -> ConfigValue:
        return ConfigValueInt("port","Port","The port for the http server to use",4999, 0, 65535)
    
    @staticmethod
    def get_show_http_debug_messages_config_value() -> ConfigValue:
        return ConfigValueBool("show_http_debug_messages","Show http debug messages","Display the JSON going in and out of the server in MantellaSoftware's log", False)
    
    #Debugging
    @staticmethod
    def get_debugging_config_value() -> ConfigValue:
        return ConfigValueBool("debugging","Activate debugging","Whether debugging is enabled.\nIf this is set to True, the values of all other variables in this section are ignored", False)
    
    @staticmethod
    def get_play_audio_from_script_config_value() -> ConfigValue:
        return ConfigValueBool("play_audio_from_script","Play audio from script","Whether to play the generated voicelines directly from the script / exe.\nSet this value to True if testing Mantella while Skyrim is not running", True)
    
    @staticmethod
    def get_debugging_npc_config_value() -> ConfigValue:
        return ConfigValueString("debugging_npc","Debugging NPC","Selects the NPC to test.\nSet this value to None if you would instead prefer to select an NPC via the mod's spell", "Hulda")
    ...
    def get_use_default_player_response_config_value() -> ConfigValue:
        ...
        return ConfigValueBool("use_default_player_response","Use default player response",description, False)
    
    @staticmethod
    def get_default_player_response_config_value() -> ConfigValue:
        return ConfigValueString("default_player_response","Default player response","The default text sent to the LLM if use_default_player_response is enabled", "Can you tell me something about yourself?")
    ...
    def get_exit_on_first_exchange_config_value() -> ConfigValue:
        ...
        return ConfigValueBool("exit_on_first_exchange","Exit on first exchange",description, False)
    ...
    def get_add_voicelines_to_all_voice_folders_config_value() -> ConfigValue:
        ...
        return ConfigValueBool("add_voicelines_to_all_voice_folders","Add voicelines to all voice folders",description, False)
