...
class LanguageDefinitions:    
    ...
    def get_goodbye_npc_response() -> ConfigValue:
        return ConfigValueString("goodbye_npc_response","Goodbye npc response","The response the NPC gives at the end of the conversation","Safe travels",tags=[ConvigValueTag.advanced])

    @staticmethod
    def get_collecting_thoughts_npc_response() -> ConfigValue:
        return ConfigValueString("collecting_thoughts_npc_response", "collecting_thoughts_npc_response","The response the NPC gives when they need to summarise the conversation because the maximum token count has been reached","I need to gather my thoughts for a moment", tags=[ConvigValueTag.advanced])
    ...
    def get_offended_npc_response() -> ConfigValue:
        ...
        return ConfigValueString("offended_npc_response","Offended npc response",description, "Offended", tags=[ConvigValueTag.advanced])
    ...
    def get_forgiven_npc_response() -> ConfigValue:
        description = """The keyword used by the NPC when they have forgiven the player for offending them
                        This should match what is stated in the prompt at the bottom of this config file"""
        return ConfigValueString("forgiven_npc_response","forgiven_npc_response",description,"Forgiven", tags=[ConvigValueTag.advanced])
    ...
    def get_follow_npc_response() -> ConfigValue:
        description = """The keyword used by the NPC when they are willing to become a follower
                        This should match what is stated in the prompt at the bottom of this config file"""
        return ConfigValueString("follow_npc_response","follow_npc_response",description,"Follow", tags=[ConvigValueTag.advanced])