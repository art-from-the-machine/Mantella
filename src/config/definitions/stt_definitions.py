    ...
    def get_whisper_url_config_value() -> ConfigValue:
        description = """The external Whisper service URL Mantella will connect to (if 'External Whisper Service' is enabled).
                        ...
                        Known services:
                            ...
                            Player2: Uses the Player2 speech-to-text service (https://player2.game). Requires the Player2 app to be running locally or a valid API key saved in secret_keys.json. The API key is obtained automatically if the Player2 app is running, or can be generated via the authentication panel in the LLM tab."""
        # --- PLAYER2 START ---
        # "Player2" added as a supported STT service.
        # It uses the POST /stt/transcribe endpoint which accepts raw PCM audio bytes,
        # matching Mantella's existing post-VAD transcription pattern.
        return ConfigValueSelection("whisper_url", "Whisper Service", description, "OpenAI", ["OpenAI", "Groq", "whisper.cpp", "Player2"], allows_free_edit=True, tags=[ConfigValueTag.advanced])
        # --- PLAYER2 END ---
